...
2. **Owner-side publishing alignment** — The highest-leverage remaining OSS-release blocker is still the canonical public identity split captured in `_dev/MODULE_PATH_DECISION.md`. The current evidence supports a clean short-term policy: public release/install surfaces stay on `gridlhq/allyourbase` and `install.allyourbase.io`, while `go.mod` stays frozen at `github.com/allyourbase/ayb` until an owner-approved module-path migration exists.

3. **Public Docker publication posture** — GHCR is now narrowed to a concrete package-settings problem, not a vague secret mystery: `ghcr.io/gridlhq/allyourbase` exists, is still private, and the latest public Docker run failed with GHCR `403`. Follow the runbook in `_dev/RELEASE_SECRETS_AUDIT.md` before restoring any Docker-install story.

4. **Browser-test lint debt cleanup** — The dedicated browser-test ESLint sweep is now wired and passing with `0` errors, but it still reports a large warning backlog (conditional assertions, intentional skips, legacy patterns). Clean that debt down so browser-test lint becomes a more useful signal.

5. **Production proof expansion** — Keep extending real end-to-end proof around newly landed routes and operator workflows so the green audit ledger stays honest rather than drifting behind feature work.

## P2 — Important, Not Urgent

6. **Production confidence testing** — Load testing (k6 suite exists in `tests/load/`), fuzz-hardening (RPC deserialization, MIME validation still pending), chaos testing (Postgres restart, storage failover, clock skew), accessibility scanning (zero a11y tests).
